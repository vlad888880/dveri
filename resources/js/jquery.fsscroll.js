$(document).ready(()=>(function($) {
  ...
  FsScroll.prototype = {
    ...
    init: function() {
      ...
      this.index =  Math.ceil($dom.scrollTop()/window.innerHeight);
    ...
    _scrollPage: function(init) {
      ...
      var self = this,
          dest = {top: 0, left: 0};
      for (var i = 1; i <= self.index; ++i) {
        var section = self.section.eq(i);
        dest = {top: dest.top + section.outerHeight(), left: dest.left + section.outerWidth()};
      }
      if(!dest) return;
      ...
      init &&  (self.canScroll = true);


      init &&  $("html,body").animate({ opacity: 1 }, {
      ...
      if(_prefix) {
        ...
        self.sections.css(_prefix + 'transition', 'all ' + (init === true ? 0 : self.options.duration) + 'ms ' + self.options.timing);
        ...
        self.sections.animate(animateCss, (init === true ? 0 : self.options.duration), function() {
          self.canScroll = true;
          self._afterScroll();
        });
      }
    ...
    _beforeScroll: function() {
      ...
      if(self.options.beforeScroll && $.type(self.options.beforeScroll) === 'function') {
        return self.options.beforeScroll.call(self, self.section.eq(self.index), self.index);
      } else
        return true;
    },
